...
export class SecurityRateLimitService implements OnModuleDestroy {
  ...
  async consume(scope: string, identifier: string, options: LimitOptions) {
    const cleanIdentifier = this.fingerprint(identifier || 'unknown');
  ...
  private consumeMemory(key: string, windowMs: number) {
    const now = Date.now();
