...
private async recoverDueCampaigns() {
  ...
  try {
    const result =
      await this.campaignsService.enqueueDueAndStuckCampaigns();

    if (
      result.queuedScheduled > 0 ||
      result.requeuedStuck > 0 ||
      result.quarantinedProcessing > 0
    ) {
      this.logger.log(
        `Campaign recovery queued ${result.queuedScheduled} due scheduled, ${result.requeuedStuck} stuck campaigns, and quarantined ${result.quarantinedProcessing} uncertain recipients`,
      );
    }
  } catch (error) {
    this.logger.error(
