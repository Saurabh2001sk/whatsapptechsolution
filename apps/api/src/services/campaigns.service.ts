...
const staleProcessingRecipients =
...
let quarantinedProcessing = 0;
const affectedCampaigns = new Map<string, string>();

for (const recipient of staleProcessingRecipients) {
  const quarantined =
    await this.prisma.campaignRecipient.updateMany({
      ...
      data: {
        ...
        failedAt: new Date(),
        errorMessage:
          'Delivery state is uncertain after worker interruption. Automatic retry was blocked to prevent a duplicate WhatsApp message.',
      },
    });

  if (quarantined.count === 1) {
    quarantinedProcessing += 1;
    affectedCampaigns.set(
...
for (const [campaignId, tenantId] of affectedCampaigns) {
  ...
  await this.createCampaignAuditLog({
    ...
    campaignId,
    actorUserId: null,
    action: 'CAMPAIGN_STALE_PROCESSING_QUARANTINED',
    metadata: {
      recoveredAt: now.toISOString(),
      finalStatus: counts.finalStatus,
      reason:
        'Automatic resend blocked because Meta delivery state may be uncertain after worker interruption.',
    },
  });
}
