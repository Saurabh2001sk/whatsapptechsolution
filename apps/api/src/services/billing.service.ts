...
async requestPlanChange(
  ...
  if (existingPendingRequest) {
    return {
      ...
      message:
  existingPendingRequest.planId === plan.id
    ? 'This plan request is already pending approval.'
    : `Another plan request for ${existingPendingRequest.plan.name} is already pending approval.`,
...
if (!isTrialPlan) {
    ...
    async (tx) => {
      ...
      if (lockedExistingRequest) {
        return {
          ...
          subscription: lockedExistingRequest,
        };
      }
  ...
  if (!result.created) {
    return {
      ...
      paymentRequired: true,
      message:
        result.subscription.planId === plan.id
          ? 'This plan request is already pending approval.'
          : `Another plan request for ${result.subscription.plan.name} is already pending approval.`,
  ...
  await this.notifications.sendToPlatformAdmins({
    ...
    metadata: {
      ...
      status: subscription.status,
      paymentProofStatus:
        subscription.paymentProofStatus,
    },
  });
...
async submitPaymentProof(
  ...
  const paymentPayerName = String(
    body.paymentPayerName || '',
  ).trim();

  const paymentProofNote = String(
    body.paymentProofNote || '',
  ).trim();

  const paymentAmountPaise = Number(
    body.paymentAmountPaise || 0,
  );
  ...
  if (
    paymentPayerName.length < 2 ||
    paymentPayerName.length > 80
  ) {
    throw new BadRequestException(
      'Payer name must be 2 to 80 characters.',
    );
  }
  ...
  const result = await this.prisma.$transaction(
    async (tx) => {
      ...
      if (
        ...
        throw new BadRequestException(
          `Payment proof cannot be submitted because subscription status is ${subscription.status}.`,
        );
      }
      ...
      const sameProofAlreadySubmitted =
        subscription.paymentProofStatus ===
          'PENDING_VERIFICATION' &&
        subscription.paymentReference ===
          paymentReference &&
        subscription.paymentPayerName ===
          paymentPayerName &&
        subscription.paymentAmountPaise ===
          paymentAmountPaise &&
        String(
          subscription.paymentProofNote || '',
        ) === paymentProofNote;

      if (sameProofAlreadySubmitted) {
        return {
  ...
  if (!result.changed) {
    return {
      ...
      unchanged: true,
      message:
        'This payment proof is already submitted and pending verification.',
      subscription: result.subscription,
    };
  }
  ...
  await this.recordBillingAuditLog({
    ...
    actorUserId,
    action:
      'BILLING_PAYMENT_PROOF_SUBMITTED',
    entityType: 'TenantSubscription',
    entityId: updatedSubscription.id,
    metadata: {
      planId: updatedSubscription.plan.id,
      planCode:
        updatedSubscription.plan.code,
      planName:
        updatedSubscription.plan.name,
      paymentReference,
      paymentAmountPaise,
      status:
        updatedSubscription.paymentProofStatus,
    },
  });
  ...
  await this.notifications.sendToPlatformAdmins({
    ...
    event:
      'BILLING_PAYMENT_PROOF_SUBMITTED',
    subject: `Payment proof submitted: ${updatedSubscription.plan.name}`,
    text: [
      'A tenant submitted manual payment proof.',
      `Tenant ID: ${tenantId}`,
      `Plan: ${updatedSubscription.plan.name} (${updatedSubscription.plan.code})`,
  ...
  return {
    ok: true,
    message:
      'Payment proof submitted. Admin verification is pending.',
    subscription: updatedSubscription,
  };
}
...
async approveSubscription(
  ...
  const approvedSubscription = await this.prisma.$transaction(
    async (tx) => {
      await tx.$executeRaw(
        Prisma.sql`
          SELECT pg_advisory_xact_lock(
  ...
  await this.recordBillingAuditLog({
    ...
    metadata: {
      ...
      paymentProofStatus:
        approvedSubscription.paymentProofStatus,
      paymentReference:
        approvedSubscription.paymentReference,
      paymentAmountPaise:
        approvedSubscription.paymentAmountPaise,
    },
  });
...
async cancelSubscription(
  ...
  const result = await this.prisma.$transaction(
    async (tx) => {
      ...
      if (
        ...
        throw new BadRequestException(
          `Only PENDING_APPROVAL subscription requests can be canceled from this action. Current status is ${subscription.status}.`,
        );
      }
  ...
  await this.recordBillingAuditLog({
    tenantId:
      canceledSubscription.tenant.id,
    actorUserId,
    action:
    ...
    metadata: {
      planId:
        canceledSubscription.plan.id,
      planCode:
        canceledSubscription.plan.code,
      planName:
...
async assertCanCreateAutomationInTransaction(
  tx: Prisma.TransactionClient,
  tenantId: string,
  automationsToAdd = 1,
) {
  if (
    !Number.isInteger(automationsToAdd) ||
    automationsToAdd < 1 ||
    automationsToAdd > 100
  ) {
    throw new BadRequestException(
      'Automation count must be between 1 and 100.',
    );
  }
  ...
  if (!subscription) {
    throw new BadRequestException(
      'Active subscription is required before creating automation workflows.',
    );
  }

  const currentAutomationCount = await tx.dripWorkflow.count({
    where: {
      tenantId,
      status: {
        not: 'ARCHIVED',
  ...
  const finalAutomationCount =
    currentAutomationCount + automationsToAdd;

  if (
    finalAutomationCount >
    subscription.plan.maxAutomationRules
  ) {
    throw new BadRequestException(
      `Your plan allows ${subscription.plan.maxAutomationRules} active automation workflows. You currently have ${currentAutomationCount}.`,
    );
  }
...
async assertCanCreateAutomation(
  tenantId: string,
  automationsToAdd = 1,
) {
  return this.prisma.$transaction(
    async (tx) =>
      this.assertCanCreateAutomationInTransaction(
        tx,
        tenantId,
        automationsToAdd,
      ),
