...
const message = this.sanitizeWebhookError(
...
async replayWebhookEvent(
  ...
  return this.prisma.$transaction(
    async (tx) => {
      ...
      const body =
        webhookEvent.payload as MetaWebhookBody;
...
private async processWebhookBody(
  ...
  const inboundMessages =
    this.extractInboundMessages(body);
...
for (const statusEvent of statusEvents) {
  const campaignResult =
    await this.syncCampaignRecipientDeliveryStatus({
      ...statusEvent,
      tenantId,
    });

  if (campaignResult.synced) {
    synced += 1;
    continue;
  }

  const dripResult =
    await this.syncDripMessageDeliveryStatus({
      ...statusEvent,
      tenantId,
    });

  if (dripResult.synced) {
    synced += 1;
  } else {
    ignoredCount += 1;
  }
...
return {
 ok: true,
 processed: statusEvents.length + inboundMessages.length,
 synced,
 ignoredCount,
 ignored: synced === 0,
};
...
private async syncCampaignRecipientDeliveryStatus(input: {
  ...
  if (
    input.status !== 'FAILED' &&
    (statusRank[input.status] ?? 0) <
      (statusRank[recipient.status] ?? 0)
  ) {
    return {
      synced: false,
      reason: 'Ignored campaign delivery status downgrade',
    };
  }
  ...
  if (
    ...
    return {
      synced: false,
      reason: 'Ignored failed status after campaign message was read',
    };
  }
...
private async syncDripMessageDeliveryStatus(input: {
  tenantId: string | null;
  messageId: string;
  status: string;
  timestamp?: string;
  ...
  if (!input.tenantId) {
    return {
      synced: false,
      reason: 'Webhook tenant could not be resolved',
    };
  }

  const eventAt = this.parseWebhookTimestamp(
    input.timestamp,
  );
  ...
  if (!dripMessage) {
    return {
      synced: false,
      reason:
        'Message ID not linked to a drip message',
    };
  }
  ...
  if (
    input.status !== 'FAILED' &&
    (statusRank[input.status] ?? 0) <
      (statusRank[dripMessage.status] ?? 0)
  ) {
    return {
      synced: false,
      reason:
  ...
  const data: Prisma.DripMessageUpdateInput = {
    status: input.status,
    statusWebhookAt: new Date(),
  };
  ...
  if (input.status === 'READ') {
    data.readAt = eventAt;
    data.deliveredAt =
      dripMessage.deliveredAt || eventAt;
    data.sentAt =
      dripMessage.sentAt || eventAt;
    data.errorMessage = null;
  }
  ...
  await this.prisma.$transaction([
    this.prisma.dripMessage.update({
      where: {
        id: dripMessage.id,
      },
      data,
    ...
    this.prisma.dripAuditLog.create({
      data: {
        tenantId: dripMessage.tenantId,
        workflowId:
          dripMessage.workflowId,
        contactId: dripMessage.contactId,
        actorUserId: null,
        action: `DRIP_MESSAGE_${input.status}`,
        metadata: {
          dripMessageId: dripMessage.id,
          enrollmentId:
            dripMessage.enrollmentId,
          metaMessageId:
            input.messageId,
...
private sanitizeWebhookError(value: unknown) {
  return String(value || 'Webhook processing failed')
    .replace(
      /Bearer\s+[A-Za-z0-9._~+/=-]+/gi,
      'Bearer [REDACTED]',
    )
